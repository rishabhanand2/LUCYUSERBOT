...
async def get_user_from_event(event, mafiaevent=None, secondgroup=None):
    ...
    if event.reply_to_msg_id:
        ...
        if previous_message.from_id is None and not event.is_private:
            await delete_mafia(mafiaevent, "`Wɘɭɭ tʜʌt's ʌŋ ʌŋoŋƴɱoʋs ʌɗɱɩŋ !`")
            return None, None
        ...
        if user.isnumeric():
            user = int(user)
        if not user:
            await delete_mafia(mafiaevent, "`Pʌss tʜɘ ʋsɘʀ's ʋsɘʀŋʌɱɘ, ɩɗ oʀ ʀɘpɭƴ !`", 5)
            return None, None
        ...
        try:
            ...
            await delete_mafia(mafiaevent, "`Coʋɭɗŋ't ʆɘtcʜ ʋsɘʀ to pʀoccɘɗ ʆʋʀtʜɘʀ !`", 5)
            return None, None
    return user_obj, extra
