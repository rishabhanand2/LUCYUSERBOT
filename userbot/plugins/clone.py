...
DEFAULTUSERBIO = str(BIO_MSG) if BIO_MSG else "Sʌŋĸɩɩ Bot Oŋ Fɩʀɘ :3 <3 ||••"
...
async def _(event):
    ...
    await borg.send_message(
        event.chat_id, "**Wʜo Aʀɘ Yoʋ ? .. **", reply_to=reply_message
    )
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
            f"#CLONED\nSʋccɘsʆʋɭɭɘƴ cɭoŋɘɗ [{first_name}](tg://user?id={user_id })",
        )
...
async def _(event):
    ...
    await event.edit("Sʋccɘsʆʋɭɭƴ ʀɘvɘʀtɘɗ to ƴoʋʀ ʌccoʋŋt ɓʌcĸ")
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID, f"#REVERT\nSʋccɘsʆʋɭɭƴ ʀɘvɘʀtɘɗ ɓʌcĸ to ƴoʋʀ pʀoʆɩɭɘ"
        )
